...
use crate::state::{
    VotedValidatorInfo, VALIDATORS, VALIDATOR_PERIODS, VALIDATOR_SLOPE_CHANGES, VALIDATOR_VOTES,
};
...
pub(crate) fn cancel_user_changes(
    ...
    let last_pool_period =
        fetch_last_pool_period(storage, period, validator_addr)?.unwrap_or(period);
...
pub(crate) fn vote_for_pool(
    ...
    update_validator_info(storage, period, pool_addr, Some((bps, vp, slope, Operation::Add)))
        .map(|_| ())
}
...
pub(crate) fn update_validator_info(
    ...
    let pool_info = match get_pool_info_mut(storage, period, validator_addr)? {
            ...
            if changes.is_some() =>
        {
            if let Some((bps, vp, slope, op)) = changes {
                ...
                pool_info.vamp_amount = op.calc_voting_power(pool_info.vamp_amount, vp, bps);
            }
            VALIDATOR_PERIODS.save(storage, (validator_addr, period_key), &())?;
            VALIDATOR_VOTES.save(storage, (period_key, validator_addr), &pool_info)?;
            pool_info
        },
...
pub(crate) fn get_pool_info_mut(
    ...
    let pool_info_result = if let Some(pool_info) =
        VALIDATOR_VOTES.may_load(storage, (period, pool_addr))?
    {
        ...
        let pool_info_result =
            if let Some(mut prev_period) = fetch_last_pool_period(storage, period, pool_addr)? {
                let mut pool_info = VALIDATOR_VOTES.load(storage, (prev_period, pool_addr))?;
                ...
                for (recalc_period, scheduled_change) in scheduled_slope_changes {
                    pool_info = VotedValidatorInfo {
                        vamp_amount: calc_voting_power(
                            pool_info.slope,
                            pool_info.vamp_amount,
                            prev_period,
                            recalc_period,
                        ),
                    ...
                    VALIDATOR_PERIODS.save(storage, (pool_addr, recalc_period_key), &())?;
                    VALIDATOR_VOTES.save(storage, (recalc_period_key, pool_addr), &pool_info)?;
