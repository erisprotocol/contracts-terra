...
use eris::voting_escrow::{ConfigResponse, Cw20HookMsg, ExecuteMsg, LockInfoResponse, QueryMsg};
...
fn lock_unlock_logic() {
    ...
    helper.create_lock(router_ref, "user", WEEK * 2, 90f32).unwrap();
...
fn random_token_lock() {
    ...
    router.execute_contract(helper.owner.clone(), random_token.clone(), &msg, &[]).unwrap();
    ...
    let err =
        router.execute_contract(Addr::unchecked("user"), random_token, &cw20msg, &[]).unwrap_err();
...
fn check_blacklist() {
    ...
    helper.create_lock(router_ref, "user1", WEEK, 10f32).unwrap();
}
