...
use eris::governance_helper::{get_period, get_periods_count, EPOCH_START, MAX_LOCK_TIME, WEEK};
...
impl Simulator {
    ...
    fn mint(&mut self, user: &str, amount: u128) {
        self.helper.mint_xastro(&mut self.router, user, amount as u64)
    }
    ...
    fn app_next_period(&mut self) {
        ...
        self.router.update_block(|block| block.time = block.time.plus_seconds(WEEK));
    }
    ...
    fn create_lock(&mut self, user: &str, amount: f64, interval: u64) -> Result<AppResponse> {
        ...
        self.helper.create_lock(&mut self.router, user, interval, amount as f32).map(|response| {
            self.add_point(
            ...
            self.locked.extend(vec![(user.to_string(), (amount, block_period + periods_interval))]);
            response
        })
    }

    fn increase_time(&mut self, user: &str, interval: u64) -> Result<AppResponse> {
        self.helper.extend_lock_time(&mut self.router, user, interval).map(|response| {
            let cur_period = self.block_period() as usize;
            let periods_interval = get_periods_count(interval);
            let end = if let Some(point) = self.get_user_point_at(cur_period, user) {
                point.end
            } else {
                let prev_point = self.get_prev_point(user).expect("We always need previous point!");
                prev_point.end
            };
            ...
            self.add_point(cur_period, user, apply_coefficient(amount, dt), end + periods_interval);
            response
        })
    }

    fn extend_lock(&mut self, user: &str, amount: f64) -> Result<AppResponse> {
        self.helper.extend_lock_amount(&mut self.router, user, amount as f32).map(|response| {
            let cur_period = self.block_period() as usize;
            let (user_balance, end) = if let Some(point) = self.get_user_point_at(cur_period, user)
            {
    ...
    fn withdraw(&mut self, user: &str) -> Result<AppResponse> {
        self.helper.withdraw(&mut self.router, user).map(|response| {
    ...
    fn append2blacklist(&mut self, user: &str) -> Result<AppResponse> {
        self.helper.update_blacklist(&mut self.router, Some(vec![user.to_string()]), None).map(
            |response| {
                let cur_period = self.block_period();
                self.add_point(cur_period as usize, user, 0.0, cur_period);
                response
    ...
    fn remove_from_blacklist(&mut self, user: &str) -> Result<AppResponse> {
        self.helper.update_blacklist(&mut self.router, None, Some(vec![user.to_string()])).map(
            |response| {
                let cur_period = self.block_period() as usize;
    ...
    fn event_router(&mut self, user: &str, event: Event) {
        ...
        let contract_balance =
            self.helper.query_user_vp(&mut self.router, user).unwrap_or(0.0) as f64;
    ...
    fn calc_total_balance_at(&mut self, period: usize) -> f64 {
        self.users
            .clone()
            .iter()
            .fold(0.0, |acc, user| acc + self.get_user_point_at(period, user).unwrap().amount)
    }
}
...
fn generate_cases() -> impl Strategy<Value = (Vec<String>, Vec<(usize, String, Event)>)> {
    ...
    users_strategy.prop_flat_map(|users| {
        (
            Just(users.clone()),
            prop::collection::vec(
                (1..=MAX_PERIOD, prop::sample::select(users), events_strategy()),
                0..MAX_EVENTS,
            ),
        )
    })
...
fn exact_simulation() {
    ...
    for period in 1..=MAX_PERIOD {
        ...
        let real_balance = simulator.calc_total_balance_at(period);
        let contract_balance =
            simulator.helper.query_total_vp(&mut simulator.router).unwrap_or(0.0) as f64;
        if (real_balance - contract_balance).abs() >= 10e-3 {
