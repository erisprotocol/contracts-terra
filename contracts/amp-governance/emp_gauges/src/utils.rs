...
#[derive(Debug)]
pub(crate) enum VotedValidatorInfoResult {
...
pub(crate) fn vote_for_validator(
    ...
    update_validator_info(
        ...
        period,
        validator_addr,
        Some((BasicPoints::max(), vp, slope, Operation::Add)),
    )
    .map(|_| ())
}
...
pub(crate) fn update_validator_info(
    ...
    let validator_info = match get_validator_info_mut(storage, period, validator_addr)? {
        VotedValidatorInfoResult::Unchanged(mut validator_info)
        | VotedValidatorInfoResult::New(mut validator_info)
            if changes.is_some() =>
        {
        ...
        VotedValidatorInfoResult::New(validator_info) => {
            VALIDATOR_PERIODS.save(storage, (validator_addr, period_key), &())?;
            VALIDATOR_VOTES.save(storage, (period_key, validator_addr), &validator_info)?;
            validator_info
        },
        VotedValidatorInfoResult::Unchanged(validator_info) => validator_info,
    };
...
pub(crate) fn get_validator_info_mut(
    ...
    {
        VotedValidatorInfoResult::Unchanged(validator_info)
    } else {
        ...
        VotedValidatorInfoResult::New(validator_info_result)
    };
